        ...
        <div class="mt-14 rounded-lg p-4">
            ...
            <div class="block rounded-lg border border-gray-200 bg-white p-6 shadow dark:border-gray-700 dark:bg-gray-800">
                <div class="relative overflow-x-auto">
                            ...
                            @foreach ($messages as $message)
                                @if ($loop->last)
                                    <tr class="odd:bg-white even:bg-gray-50 odd:dark:bg-gray-900 even:dark:bg-gray-800">
                                        ...
                                        <td class="px-6 py-4">
                                            <form id="deleteForm-{{ $message->id }}" action="{{ route('admin.contact.destroy', ['contact' => $message->id]) }}" method="POST" class="inline-block">
                                                @method('delete')
                                                @csrf
                                                <button type="button" onclick="return confirmDelete({{ $message->id }})" class="font-medium text-blue-600 hover:underline dark:text-blue-500">Hapus</button>
                                            </form>
                                        </td>
                                    </tr>
                                @else
                    ...
                    {{ $messages->onEachSide(0)->links('Admin.Partials.Pagination') }}
                </div>
            </div>
        </div>
    </div>
