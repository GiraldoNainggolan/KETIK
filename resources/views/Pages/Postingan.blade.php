    ...
    <div class="w-full pt-20 ">
        ...
        <div class="container grid md:grid-cols-4 grid-cols-2 gap-6 pt-4 pb-16 items-start px-8 lg:px-40">
            ...
            <div id="drawer-example"
                ...
                <div class="divide-y divide-gray-200 space-y-5">
                    <form id="filter-form" method="GET" action="{{ url()->current() }}"
                        onsubmit="updateUrlParameters(event)">
                        <div class="divide-y divide-gray-200 space-y-2">
                                    ...
                                    @foreach ($categories as $category)
                                        ...
                                        <div class="flex items-center">
                                            <input type="checkbox" name="category[]"
                                                value="{{ strtolower(strtok($category->name, ' ')) }}"
                                                id="cat-{{ $i }}"
                                                ...
                                                {{ in_array(strtolower(strtok($category->name, ' ')), request('category', [])) ? 'checked' : '' }}
                                                onchange="document.getElementById('filter-form-desktop').submit();">
                                            <label for="cat-1"
                                                class="dark:text-white ml-3 cursor-pointer">{{ $category->name }}</label>
                            ...
                            <div class="pt-4">
                                ...
                                <div class="mt-4 flex items-center">
                                    <input type="text" name="min" id="min" value="{{ request('min') }}"
                                        class="w-full border-gray-300 focus:border-primary rounded focus:ring-0 px-3 py-1 text-gray-600 shadow-sm"
                                        placeholder="min" onchange="document.getElementById('filter-form').submit();">
                                    <span class="mx-3 text-gray-500">-</span>
                                    <input type="text" name="max" id="max" value="{{ request('max') }}"
                                        class="w-full border-gray-300 focus:border-primary rounded focus:ring-0 px-3 py-1 text-gray-600 shadow-sm"
                                        placeholder="max" onchange="document.getElementById('filter-form').submit();">
                                </div>
                            </div>
                            ...
                            <div class="pt-4">
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="size-selector">
                                        <input type="radio" name="rating" value="1" id="size-xs"
                                            class="hidden" {{ request('rating') == '1' ? 'checked' : '' }}
                                    ...
                                    <div class="size-selector">
                                        <input type="radio" name="rating" value="2" id="size-sm"
                                            class="hidden" {{ request('rating') == '2' ? 'checked' : '' }}
                                            onchange="document.getElementById('filter-form').submit();">
                                        <label for="size-sm"
                                    ...
                                    <div class="size-selector">
                                        <input type="radio" name="rating" value="3" id="size-m"
                                            class="hidden" {{ request('rating') == '3' ? 'checked' : '' }}
                                            onchange="document.getElementById('filter-form').submit();">
                                        <label for="size-m"
                                    ...
                                    <div class="size-selector">
                                        <input type="radio" name="rating" value="4" id="size-l"
                                            class="hidden" {{ request('rating') == '4' ? 'checked' : '' }}
                                            onchange="document.getElementById('filter-form').submit();">
                                        <label for="size-l"
                                    ...
                                    <div class="size-selector">
                                        <input type="radio" name="rating" value="5" id="size-xl"
                                            class="hidden" {{ request('rating') == '5' ? 'checked' : '' }}
                                            onchange="document.getElementById('filter-form').submit();">
                                        <label for="size-xl"
                            ...
                            <div class="pt-4">
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-center">
                                        <input type="checkbox" name="availability[]" value="tersedia" id="tersedia"
                                            class="focus:ring-0 rounded-sm cursor-pointer dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600"
                                            {{ in_array('tersedia', request('availability', [])) ? 'checked' : '' }}
                                            onchange="document.getElementById('filter-form').submit();">
                                        <label for="tersedia" class="dark:text-white ml-3 cursor-pointer">Tersedia</label>
                                        {{-- <div class="ml-auto dark:text-white text-sm">(15)</div> --}}
                                    </div>
                                    <div class="flex items-center">
                                        <input type="checkbox" name="availability[]" value="habis" id="habis"
                                            class="focus:ring-0 rounded-sm cursor-pointer dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600"
                                            {{ in_array('habis', request('availability', [])) ? 'checked' : '' }}
                                            onchange="document.getElementById('filter-form').submit();">
            ...
            <div class="col-span-1 bg-white dark:bg-gray-700 px-4 pb-6 shadow rounded overflow-hiddenb hidden md:block">
                <div class="divide-y divide-gray-200 space-y-5">
                    <form id="filter-form-desktop" method="GET" action="{{ url()->current() }}"
                        onsubmit="updateUrlParameters(event)">
                        <div class="divide-y divide-gray-200 space-y-5">
                            <!-- Filter Kategori -->
                            <div>
                ...
                <div class="flex justify-between mb-4">
                    <form id="sortForm" class="w-40px" method="GET" action="{{ url()->current() }}">
                        <select id="sort" name="sort"
                            ...
                            <option class="py-2" value="default" {{ request('sort') == 'default' ? 'selected' : '' }}>
                                Default</option>
                            <option class="py-2" value="terbaru" {{ request('sort') == 'terbaru' ? 'selected' : '' }}>
                                Terbaru</option>
                            <option class="py-2" value="terlama" {{ request('sort') == 'terlama' ? 'selected' : '' }}>
                                Terlama</option>
                        </select>
                    </form>
                    ...
                    <div class="flex gap-2 ml-auto">
                        <div @click="setLayout('grid')"
                            :class="{ 'bg-blue-700 text-white': layout === 'grid', 'dark:bg-gray-600 bg-white': layout !== 'grid' }"
                            class="w-10 h-9 flex items-center justify-center rounded cursor-pointer">
                            <i class="fa-solid fa-grip-vertical"></i>
                        </div>
                        <div @click="setLayout('list')"
                            :class="{ 'bg-blue-700 text-white': layout === 'list', 'dark:bg-gray-600 bg-white': layout !== 'list' }"
                            class="border border-gray-300 w-10 h-9 flex items-center justify-center  rounded cursor-pointer">
                ...
                <div :class="layout === 'grid' ? 'grid md:grid-cols-3 grid-cols-2 gap-6' : 'flex flex-col gap-4'">
                    ...
                    @if ($products->count() > 0)
                        @foreach ($products as $product)
                            <div
                                :class="layout === 'grid' ?
                                    'bg-white dark:bg-gray-600 shadow rounded overflow-hidden group' :
                                    'flex items-center p-4 bg-white border border-gray-200 dark:bg-gray-700 dark:border-gray-700 rounded-lg shadow-md'">
                                <div class="relative ">
                                    <img src="{{ asset('storage/' . $product->photo) }}" alt="{{ $product->name }}"
                                        class="w-full h-24 lg:h-48 object-cover">
                                    <div
                                        :class="layout === 'grid' ?
                                            'absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition ' :
                                            'inset-0 flex gap-2 mt-2'">
                                ...
                                <div :class="layout === 'list' ? 'ml-4' : 'pt-5 pb-3 px-2 lg:px-4'">
                                    <p>
                                        @if ($product->status == 'tersedia')
                                            <span
                                                class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{{ $product->status }}</span>
                                        @else
                                            <span
                                                class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-red-400 border border-red-400">{{ $product->status }}</span>
                                        @endif
                                    <h4
                                        class="uppercase dark:text-white font-bold text-sm lg:text-xl mb-2 text-gray-800 hover:text-blue-800 transition">
                                        {{ $product->name }}</h4>
                                    </p>
                                    <div class="flex items-baseline mb-1 space-x-2">
                                        <p class="text-base lg:text-xl dark:text-white font-semibold">Rp
                                            {{ number_format($product->price, 0, ',', '.') }}</p>
                                    </div>

                                    <div class="flex flex-col gap-2 mt-2 text-black dark:text-white">
                                        <a target="_blank" class="flex items-center gap-2 font-medium"
                                            href="{{ route('merch-info', $product->merchant->id) }}">
                                            <p
                                                class="text-[10px] lg:text-xs font-light text-blue-800 dark:text-blue-300 lg:line-clamp-none line-clamp-2">
                                    ...
                                    <div class="flex lg:items-center mt-2 flex-col lg:flex-row ">
                                        ...
                                        <div class="flex gap-1 text-sm text-yellow-400 ">
                                            @for ($i = 0; $i < $fullStars; $i++)
                                                <svg aria-hidden="true" class="w-3 lg:w-5 h-3 lg:h-5 text-yellow-400"
                                        ...
                                        <span
                                            class="m-0 lg:ml-1 text-gray-500 dark:text-gray-400">{{ $product->rating }}</span>
                                        <div class="text-xs text-gray-500 m-0 lg:ml-3">
                                            {{ $product['getReview']->count() }} Review</div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
